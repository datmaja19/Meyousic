...
public class RecentFragment extends Fragment {
    ...
    RecyclerView recent_recycleview;
    Context context;
    public List<SongModel> lisrecent = new ArrayList<>();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (context instanceof MainActivity) {
            lisrecent=    ((MainActivity)context).getrecent();



        }

        recent_recycleview=view.findViewById(R.id.recent_rv);
        context=getContext();
        recent_adapter = new RecentAdapter(context,lisrecent);
        LinearLayoutManager layoutManager1 = new LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false);
        recent_recycleview.setLayoutManager(layoutManager1);
        recent_recycleview.setAdapter(recent_adapter);
